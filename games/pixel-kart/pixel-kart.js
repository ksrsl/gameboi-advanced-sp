...
const RACERS = [
  { name: 'NOVA', color: '#f3f5f6', trim: '#15191d', speed: 83.2 },
  { name: 'VEX', color: '#506f86', trim: '#e7edf0', speed: 81.4 },
  { name: 'BYTE', color: '#b6bec5', trim: '#252b31', speed: 80.2 },
  { name: 'LUX', color: '#252b31', trim: '#e9edf0', speed: 84.1 },
  { name: 'JET', color: '#748693', trim: '#111519', speed: 79.4 }
];

export default {
  id: 'pixel-kart', title: 'Pixel Kart', version: '1.0.0',
  create() {
    ...
    const markup = () => `<div class="kart-game">
      ...
      <div class="kart-overlay" id="kart-overlay"><strong>PIXEL KART</strong><small>KSR NIGHT CIRCUIT<br>THREE LAPS - SIX RACERS - ITEM BOXES</small><button data-kart="start">START RACE</button><em>LEFT / RIGHT STEER - A ITEM - HOLD B DRIFT</em></div>
    ...
    function curveAt(distance) { return Math.sin(distance * .0047) * 42 + Math.sin(distance * .0113) * 17; }
    ...
    function resetRace() {
      ...
      rivals = RACERS.map((racer, index) => ({ ...racer, total: 18 + index * 14, offset: [-.55, .42, -.15, .65, .1][index], targetOffset: [-.55, .42, -.15, .65, .1][index], wobble: index * 1.7 }));
    ...
    function showOverlay(title, copy, button) {
      ...
      overlay.innerHTML = `<strong>${title}</strong><small>${copy}</small><button data-kart="start">${button}</button><em>LEFT / RIGHT STEER - A ITEM - HOLD B DRIFT</em>`;
    }

    function renderUi() {
      root.querySelector('#kart-pause').hidden = state !== 'pause';
      if (state === 'title') showOverlay('PIXEL KART', `KSR NIGHT CIRCUIT<br>THREE LAPS - SIX RACERS - BEST ${bestTime ? formatTime(bestTime / 1000) : '--:--'}`, 'START RACE');
    ...
    function finish() {
      ...
      if (finishPosition === 1 && (!bestTime || raceTime * 1000 < bestTime)) { bestTime = Math.round(raceTime * 1000); services.storage.set('pixelKart:bestTime', bestTime); }
      services.tone(finishPosition === 1 ? 880 : 470, .14, 'triangle'); renderUi();
    }

    function updateRivals(dt) {
      rivals.forEach((rival, index) => {
        const rubberBand = clamp((total - rival.total) * .008, -4, 5);
        const pulsePenalty = pulseTimer > 0 && rival.total > total && rival.total - total < 240 ? 14 : 0;
        rival.total += Math.max(48, rival.speed + rubberBand - pulsePenalty) * dt;
        rival.wobble += dt * (.7 + index * .06);
        if (Math.sin(rival.wobble) > .96) rival.targetOffset = clamp((Math.random() - .5) * 1.45, -.78, .78);
        rival.offset = smoothToward(rival.offset, rival.targetOffset, 1.8, dt);
      });
    ...
    function drawTrack() {
      const sky = ctx.createLinearGradient(0, 0, 0, HORIZON + 20); sky.addColorStop(0, '#06080c'); sky.addColorStop(1, '#263848'); ctx.fillStyle = sky; ctx.fillRect(0, 0, WIDTH, HEIGHT);
      ...
      ctx.fillStyle = '#0e1419'; for (let x = -5, i = 0; x < 330; i += 1) { const w = 12 + (i * 7) % 15; const h = 7 + (i * 11) % 20; ctx.fillRect(x, HORIZON - h, w, h); if (i % 3 === 0) { ctx.fillStyle = '#8dbbd155'; ctx.fillRect(x + 3, HORIZON - h + 4, 2, 3); ctx.fillStyle = '#0e1419'; } x += w - 1; }
      ctx.fillStyle = '#252b30'; ctx.fillRect(0, HORIZON, WIDTH, HEIGHT - HORIZON);
      for (let far = 420; far > 8; far -= 7) {
        const near = far - 7; const a = project(far), b = project(near);
        const band = Math.floor((total + near) / 28) % 2;
        ctx.fillStyle = band ? '#394148' : '#333a40';
        ...
        ctx.fillStyle = band ? '#eef2f4' : '#15191d';
      ...
      for (let marker = firstMarker; marker < total + 420; marker += 42) {
        ...
        const a = project(z2), b = project(z1); ctx.fillStyle = '#f5f6f7';
    ...
    function drawRoadside() {
      ...
      for (let marker = firstPost; marker < total + 400; marker += 64) {
        ...
        [-1, 1].forEach(side => {
          ...
          ctx.fillStyle = marker % 128 ? '#9be4ff' : '#f5f7f8';
          ctx.globalAlpha = .85;
          ctx.beginPath();
    ...
    function drawObjects() {
      ...
      rivals.slice().sort((a, b) => b.total - a.total).forEach(rival => {
        ...
        drawKart(p.x, p.y, clamp(p.scale * 1.05, .12, .95), rival.color, rival.trim, Math.sin(rival.wobble) * .04);
      });
    }
    ...
    return {
      async mount(host, providedServices) {
        services = providedServices; bestTime = services.storage.get('pixelKart:bestTime', 0); host.innerHTML = markup(); root = host.firstElementChild;
        ...
        root.addEventListener('click', event => { const action = event.target.closest?.('[data-kart]')?.dataset.kart; if (action === 'start') start(); if (action === 'exit') services.exit(); });
